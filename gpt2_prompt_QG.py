...
zero_shot_prompt_input = "The task is to generate a relevant [Contextual Question] from the given [Context] using the given [Answer Keywords].[SEP]" \
        "\n(1) [Context]: Bearded dragons are a species of lizards that are native to Australia. They are popular pets due to their relatively calm demeanor and interesting behaviors. Bearded dragons get their name from the spiny projections under their necks, which they can puff out to appear larger and more intimidating. These lizards are omnivores, eating both insects and vegetation, and can live up to 10 years in captivity. Used [Answer Keywords]: 'spiny projections' to generate [Contextual Question]:"
...
one_shot_prompt_no_ans_input = "The task is to generate a relevant Context Specific Question from the given [Context]. Possible answers to the generated [Contextual Question] should be relevant IN the given [Context].[SEP]" \
        ...
        "\n(2) [Context]: Bearded dragons are a species of lizards that are native to Australia. They are popular pets due to their relatively calm demeanor and interesting behaviors. Bearded dragons get their name from the spiny projections under their necks, which they can puff out to appear larger and more intimidating. These lizards are omnivores, eating both insects and vegetation, and can live up to 10 years in captivity.[Contextual Question]:"

# Change prompt type accordingly
input = k_shot_prompt_input
...
input = k_shot_prompt_input + "\n(7) [Context]: " + squad[split_type][ind]['context'] + " Used [Answer Keywords]: '" + high_light + "' to generate [Contextual Question]:"
result = generator(input, max_length=1000, num_return_sequences=10)
answers = []

print('10 sample Questions generated using original span')
for i in range(10):
    answers.append(result[i]['generated_text'].split('(7)')[1].split('[Contextual Question]:')[1].split('?')[0])
    print(answers[i])
